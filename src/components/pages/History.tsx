...
function StatusBadge({
  ...
  status: "FAKE" | "AUTHENTIC" | "SCANNING";
}) {
  return (
    <span
    ...
    <tr className="border-b border-border hover:bg-muted/20 transition-colors">
      ...
      <td className="px-4 py-4">
        <div className="flex items-center gap-2">
          <button
            onClick={onView}
            className="text-xs text-foreground border border-border px-3 py-1 rounded hover:bg-muted/40 transition-colors"
          >
            View
          </button>
          <button
            onClick={onDelete}
            className="text-xs text-red-400 border border-red-500/30 px-3 py-1 rounded hover:bg-red-500/10 transition-colors"
          >
            Delete
          </button>
        </div>
...
export function History() {
  ...
  const removeScan = useDashboardStore((s) => s.removeScan);
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-muted-foreground mt-1">
          Comprehensive log of all neural analysis operations.
        </p>
      </div>
        ...
        <>
            ...
            <table className="w-full">
              <thead>
                <tr className="border-b border-border">
                  {["Filename", "Date", "Status", "Confidence", "Actions"].map(
                    (h) => (
                      <th
                        key={h}
              ...
              <tbody>
                {filtered.map((scan) => (
                  <DesktopRow
                    key={scan.id}
                    scan={scan}
                    onView={() => navigate(`/results?id=${scan.id}`)}
          ...
          <div className="flex items-center justify-between">
            <p className="text-xs text-muted-foreground">
              Showing {filtered.length} of {scans.length} entries
            </p>
